...
def main(config):
    ...
    if config.dataset == 'submit':
        ...
        for i in range(10):
            ...
            B_reps = B_train_reps.tolist()
            B_reps.extend(B_test_reps.tolist())
            B_reps = torch.tensor(B_reps)

            A_preds = lin_last_net(B_reps)
            
            preds = np.concatenate(preds, A_preds.detach().numpy(), axis=0)
            ids = np.concatenate

            # Convert the concatenated tensor to a NumPy array
            np.savez("task2_submission.npz", ids=ids, representations=preds)
